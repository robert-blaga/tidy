---
name: tidy
description: Use when the user wants to file, organize, sort, clean up, or "tidy" files in a folder of their machine. Reads ~/.tidy/ for the user's accumulated filing rules and grows them as the user approves new ones. Trigger phrases include "tidy ~/Downloads", "organize my desktop", "file these books", "clean up this folder", or the slash command /tidy.
---

# Tidy

The user wants to file, organize, or tidy files. Their accumulated filing taste lives at `~/.tidy/` as plain markdown.

## What to do

1. **Read `~/.tidy/AGENTS.md`** — that's the full playbook. Follow it exactly.
